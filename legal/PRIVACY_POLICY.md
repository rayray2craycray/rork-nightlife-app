# Privacy Policy for Rork (Nox Social)

**Effective Date:** February 13, 2026
**Last Updated:** February 13, 2026

---

## 1. Introduction

Welcome to Rork ("we," "our," or "us"). We respect your privacy and are committed to protecting your personal data. This privacy policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application (the "App").

**By using Rork, you agree to the collection and use of information in accordance with this policy.**

---

## 2. Information We Collect

### 2.1 Information You Provide Directly

**Account Information:**
- Email address (required for registration)
- Display name
- Date of birth (to verify you are 21 years or older)
- Password (encrypted and never stored in plain text)
- Profile picture (optional)
- Phone number (optional)
- Biography (optional)

**Content You Create:**
- Photos and videos you upload
- Comments and messages
- Reviews and ratings
- Event check-ins

**Payment Information:**
- When purchasing tickets or making in-app purchases, payment information is processed by our third-party payment processor (Stripe). We do not store your credit card details on our servers.

### 2.2 Information Collected Automatically

**Location Data:**
- **Precise Location:** When you grant permission, we collect your precise GPS location to show nearby venues, events, and friends.
- **How to Control:** You can disable location access in your device settings at any time. Some features (venue discovery, check-ins) will not function without location access.

**Device Information:**
- Device type and model
- Operating system version
- Unique device identifiers
- Mobile network information
- IP address

**Usage Data:**
- App features you use
- Venues you view and visit
- Videos you watch and like
- Challenges you join
- Time spent in the app
- Crash reports and performance data

**Camera and Photo Library:**
- We access your camera and photo library only when you choose to upload content (profile pictures, video highlights, memories).
- We do not access your camera or photos without your explicit permission.

### 2.3 Information from Third Parties

**Social Media:**
- If you connect your Instagram account, we may receive your public profile information, followers list, and content you authorize us to access.

**Friends and Contacts:**
- If you grant permission, we may access your contacts to help you find friends who use Rork. We do not store your entire contact list without your consent.

---

## 3. How We Use Your Information

We use your information for the following purposes:

### 3.1 Provide and Improve Our Services
- Create and manage your account
- Show you nearby venues and events
- Enable you to upload and share content
- Connect you with friends and other users
- Process ticket purchases and event check-ins
- Send you notifications about friend requests, challenges, and events
- Provide customer support

### 3.2 Personalization
- Recommend venues and events based on your preferences
- Suggest friends and crews to join
- Customize your feed and discovery experience
- Show content from performers you follow

### 3.3 Safety and Security
- Detect and prevent fraud and abuse
- Enforce our Terms of Service
- Respond to legal requests and prevent harm
- Moderate content for community guidelines violations

### 3.4 Analytics and Research
- Understand how users interact with our app
- Analyze trends and usage patterns
- Improve app performance and fix bugs
- Develop new features

### 3.5 Communications
- Send you service-related emails (password resets, account notifications)
- Send push notifications about friends, events, and challenges (you can opt out)
- Respond to your inquiries and support requests

### 3.6 Advertising and Marketing
- Currently, we do not display third-party advertisements
- We may send promotional content about new features (you can opt out)

---

## 4. How We Share Your Information

We do not sell your personal data. We share your information only in the following circumstances:

### 4.1 With Your Consent
- When you choose to share content publicly
- When you tag friends in posts or memories
- When you join a venue's public lobby or crew

### 4.2 With Service Providers
We share data with trusted third-party service providers who help us operate our app:

**Cloudinary** - Image and video hosting
**Purpose:** Store and deliver photos and videos you upload
**Data Shared:** Uploaded media files
**Privacy Policy:** https://cloudinary.com/privacy

**MongoDB Atlas** - Database hosting
**Purpose:** Store user accounts, posts, and app data
**Data Shared:** All data you provide to the app
**Privacy Policy:** https://www.mongodb.com/legal/privacy-policy

**Heroku** - Application hosting
**Purpose:** Host our backend servers
**Data Shared:** All data processed by the app
**Privacy Policy:** https://www.salesforce.com/company/privacy/

**Sentry** - Error monitoring
**Purpose:** Track and fix app crashes and bugs
**Data Shared:** Error logs, device info, anonymized user IDs
**Privacy Policy:** https://sentry.io/privacy/

**Stripe** - Payment processing
**Purpose:** Process ticket purchases
**Data Shared:** Payment information (credit card, billing address)
**Privacy Policy:** https://stripe.com/privacy

**Expo/EAS** - App development and updates
**Purpose:** Build and deploy app updates
**Data Shared:** Minimal device info for crash reporting
**Privacy Policy:** https://expo.dev/privacy

### 4.3 For Legal Reasons
We may disclose your information if required by law or in response to:
- Court orders, subpoenas, or legal processes
- Government or law enforcement requests
- Protection of our rights, property, or safety
- Prevention of fraud or illegal activity

### 4.4 Business Transfers
If Rork is acquired by or merged with another company, your information may be transferred to the new owner.

### 4.5 Aggregated and Anonymized Data
We may share aggregated or anonymized data that cannot identify you personally, such as:
- Number of users in a city
- Popular venues and events
- General usage statistics

---

## 5. Your Privacy Rights

### 5.1 Access and Update
- You can view and update your profile information in the app settings
- You can download a copy of your data by contacting us

### 5.2 Delete Your Account
- You can delete your account at any time in app settings
- Upon deletion, we will remove your personal data within 30 days
- Some data may be retained for legal or safety reasons (e.g., banned users)

### 5.3 Opt-Out of Communications
- **Push Notifications:** Disable in app settings or device settings
- **Marketing Emails:** Unsubscribe link at the bottom of emails
- **Location Tracking:** Disable location access in device settings

### 5.4 Data Portability
- You have the right to receive a copy of your data in a machine-readable format
- Contact us at privacy@rork.app to request your data

### 5.5 GDPR Rights (European Users)
If you are in the European Economic Area (EEA), you have additional rights:
- **Right to Access:** Request a copy of your personal data
- **Right to Rectification:** Correct inaccurate data
- **Right to Erasure:** Request deletion of your data ("right to be forgotten")
- **Right to Restrict Processing:** Limit how we use your data
- **Right to Data Portability:** Receive your data in a portable format
- **Right to Object:** Object to processing of your data
- **Right to Withdraw Consent:** Withdraw consent at any time

To exercise these rights, contact us at privacy@rork.app.

### 5.6 CCPA Rights (California Users)
If you are a California resident, you have the right to:
- Know what personal data we collect about you
- Know whether we sell or disclose your personal data
- Opt out of the sale of your personal data (we do not sell your data)
- Request deletion of your personal data
- Not be discriminated against for exercising your privacy rights

To exercise these rights, contact us at privacy@rork.app.

---

## 6. Data Retention

We retain your information for as long as necessary to provide our services:

- **Account Data:** Retained while your account is active
- **Content:** Retained until you delete it or close your account
- **Usage Data:** Retained for up to 2 years for analytics
- **Legal Compliance:** Some data may be retained longer if required by law

After account deletion, we will delete or anonymize your data within 30 days, except:
- Data we must retain for legal reasons (e.g., transaction records)
- Data needed to enforce our Terms of Service (e.g., banned users)
- Backups may contain your data for up to 90 days

---

## 7. Data Security

We implement industry-standard security measures to protect your data:

**Technical Measures:**
- Encryption of data in transit (HTTPS/TLS)
- Encryption of sensitive data at rest
- Secure password hashing (bcrypt)
- Regular security audits and updates

**Organizational Measures:**
- Access controls and authentication
- Employee training on data protection
- Incident response procedures

**However, no method of transmission or storage is 100% secure. We cannot guarantee absolute security.**

---

## 8. Children's Privacy (COPPA Compliance)

**Rork is not intended for users under 21 years of age.**

We require all users to verify they are at least 21 years old during registration. We do not knowingly collect personal information from anyone under 21.

If we discover a user is under 21, we will delete their account and data immediately.

If you believe a child under 21 has created an account, please contact us at privacy@rork.app.

---

## 9. International Data Transfers

Rork is based in the United States. If you access our app from outside the U.S., your data may be transferred to, stored in, and processed in the United States.

**By using Rork, you consent to the transfer of your data to the United States.**

We take steps to ensure your data is protected in accordance with this privacy policy, regardless of where it is processed.

---

## 10. Third-Party Links

Our app may contain links to third-party websites, services, or social media platforms (e.g., Instagram, venue websites). We are not responsible for the privacy practices of these third parties.

We encourage you to review the privacy policies of any third-party services you visit.

---

## 11. Changes to This Privacy Policy

We may update this privacy policy from time to time. When we make changes:

- We will update the "Last Updated" date at the top
- For significant changes, we will notify you via email or in-app notification
- Your continued use of Rork after changes constitutes acceptance of the updated policy

We encourage you to review this policy periodically.

---

## 12. Contact Us

If you have questions, concerns, or requests regarding this privacy policy or your data:

**Email:** privacy@rork.app
**Support:** support@rork.app
**Mail:** Rork, [Your Business Address]

**Response Time:** We aim to respond to all privacy inquiries within 7 business days.

---

## 13. Data Protection Officer (EU Users)

For users in the European Union, you may contact our Data Protection Officer at:

**Email:** dpo@rork.app

---

## 14. Cookies and Tracking Technologies

**Currently, our app does not use cookies or third-party tracking.** We may add analytics tools in the future. If we do, we will update this policy and provide opt-out options.

---

## 15. Do Not Track Signals

Some browsers have a "Do Not Track" feature. Our app does not currently respond to Do Not Track signals, as there is no industry standard for handling them.

---

## 16. Your Consent

By using Rork, you consent to this privacy policy and our collection, use, and sharing of your information as described herein.

If you do not agree with this policy, please do not use our app.

---

**This privacy policy was last updated on February 13, 2026.**

---

## Summary (Plain Language)

**What we collect:**
- Your email, name, and date of birth
- Your location (to show nearby venues)
- Photos and videos you upload
- How you use the app

**Why we collect it:**
- To show you nearby venues and friends
- To let you upload and share content
- To make the app better

**Who we share with:**
- Service providers who help run the app (Cloudinary, MongoDB, Stripe)
- Law enforcement if required by law
- We do NOT sell your data to advertisers

**Your rights:**
- View and update your info
- Delete your account anytime
- Turn off location tracking
- Opt out of notifications

**Questions?** Email us at privacy@rork.app

---

**End of Privacy Policy**
